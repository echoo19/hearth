...
export const COMPONENT_DOCS: Record<ComponentType, string> = {
  ...
  CharacterController:
    "Input-driven movement as data: mode 'topDown' (drives both axes) or 'platformer' (drives x, leaves y to gravity), with speed, acceleration, drag, jumpHeight (pixels, converted using scene gravity), coyoteFrames, jumpBufferFrames, maxFallSpeed, airControl, and the action names to read. Set `axes` to virtual axis names (inputMappings.axes) for analog steering, so a half-pushed stick moves at half speed — needed for twin-stick and gamepad games; digital actions and axes can both be configured and the larger magnitude wins. Diagonals are clamped to one speed. Writes PhysicsBody.velocity BEFORE scripts run, so onUpdate can overwrite velocity to layer game-specific feel (dash, drift, wall-jump) on top. Needs a PhysicsBody. Set enabled=false to hand movement back to scripts without removing it. Do NOT use it for motion that is not input-driven: cutscene movers, AI-steered enemies, vehicles with custom physics.",
  Health:
