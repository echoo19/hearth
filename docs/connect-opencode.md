...
Open a project, focus the **Agent** panel, and type `opencode`. The shell starts
at the project root with `hearth` already on PATH. Hearth does not detect
OpenCode or Ollama and does not write `opencode.json`; use the steps below for
optional MCP and local-model configuration.
