...
- `skills/<slug>/`: your skills, one folder each ([agents.md](./agents.md)).
- `skills.json`: which of them are switched off.
- `models.json`: models you hid from the selector.
- `permissions.json`: each project's permission mode, one entry per folder.
  Kept here rather than in the project so a repo you push never carries it
  ([agents.md](./agents.md#permission-modes)).

Electron itself keeps a small amount of app state in the platform default
(`~/Library/Application Support/Hearth` on macOS, `%APPDATA%\Hearth` on
Windows, `~/.config/Hearth` on Linux) along with the updater's download cache.
Deleting those loses nothing but window state and a pending download.
