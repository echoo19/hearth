...
writes them into the asset's own `metadata` — no new asset, no new file.
...
Import a `.ttf`/`.otf`/`.woff`/`.woff2` file (`importAsset` — inferred
...
under exactly that name — no CSS `@font-face` block to write, no
guessing at font-family strings. A single font's load failure logs a
