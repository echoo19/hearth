...
- **The marker field**: an entity that is a live instance of a prefab
  ...
  It's an entity-level field, not a component, so it won't show up in
...
- every prefab local that still exists keeps the **same scene id** it
  already had (via the marker's `ids` map), so entity references elsewhere
...
- **Add to scene** (Assets panel, on a prefab asset's card): calls
  ...
  viewport center.
- **Update prefab** (Inspector; shown as a banner reading "Instance of
  `<name>`" when the selected entity carries a `prefab` marker): calls
