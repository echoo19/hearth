...
`hearth validate` / `validate_project` reports these stable issue codes.
Some are errors, some are warnings; branch on `errors[]` vs `warnings[]`
for severity:

`ANIMATION_FRAME_NOT_FOUND`, `BODY_WITHOUT_COLLIDER`,
`COLLIDER_COLLIDES_WITH_NOTHING`, `COLLIDES_WITH_UNKNOWN_LAYER`,
`DUPLICATE_ENTITY_ID`, `DUPLICATE_SCENE_NAME`, `EMPTY_SCRIPT_PATH`,
`FRAME_NOT_FOUND`, `ICON_ASSET_NOT_IMAGE`,
`INVALID_ANIMATION_ASSET_TYPE`, `LINERENDERER_TOO_FEW_POINTS`,
`MISSING_ANIMATION_ASSET`, `MISSING_ASSET_FILE`, `MISSING_AUDIO_ASSET`,
`MISSING_ICON_ASSET`, `MISSING_INITIAL_SCENE`, `MISSING_PARENT`,
