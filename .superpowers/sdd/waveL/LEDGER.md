...
- Disposition: **fixed 94033b5 (round 2)** — root cause found and removed after the shell re-auditor (SHELL-1) captured the real stack; a752c69's Graphics guards kept (they cover a different, app-level mechanism). Round-2 detail below, original round-1 investigation preserved after it.

  Round 2 (L-116 round-2 fixer, after SHELL-1 captured the stack):
  - Root cause: `PixiSceneView.destroy()` called `app.destroy(true, { children: true })`. In Pixi v8, `true` as rendererDestroyOptions means BOTH `removeView` AND `releaseGlobalResources`; the latter runs `GlobalResourceRegistry.release()`, which destroys Pixi's **module-global batch pool** (`batchPool` in rendering/batcher/shared/Batcher.js) shared by every Application on the page. Two compounding pool properties make that fatal to *other* renderers: `getBatchFromPool()` never nulls the array slot it pops (stale refs to checked-out batches), and the registry `clear()` sweeps the whole array (`for (const item of batchPool) item.destroy()` → `batch.textures = null`) — destroying Batch objects a *live* renderer's Batcher still references. That renderer's next instruction rebuild returns its (now-destroyed) batches to the pool, `Batcher.break()` re-acquires one, and `textureBatch.clear()` throws "Cannot read properties of null (reading 'clear')" — exactly SHELL-1's captured stack (`_DefaultBatcher2.break` line = `batch.textures.clear()` on a fresh `getBatchFromPool()` result).
  - Trigger at our layer: React StrictMode double-mounts GamePreview; both effect runs start `mountGameView` concurrently; the cancelled first mount's view resolves late and hits `if (cancelled) view.destroy()` (GamePreview.tsx) → `app.destroy(true, …)` while the surviving view is live. The crash needs the survivor to have REBUILT instructions ≥2 times before the doomed destroy (batches only transit the pool on rebuild — `RenderGroupSystem` skips `_buildInstructions` for static scenes, which is why round 1's instrumented `Graphics.clear` harness saw nothing), hence ~15% of sessions, idle included (mount-time syncEntities/particle churn keeps structure changing). Also consistent with B5's rapid project switching (old view destroyed while new one renders) and B2's close-time `_buildInstructions` pageerror.
  - Proof of trigger: patched `Application.prototype.init/destroy/render` in the page's shared pixi module instance (versioned `?v=` dep URL) and re-ran sessions — the crash session shows exactly: app#1 init, app#2 init, app#2 renders 2 frames, app#1 DESTROY rendererOpts=true (stack: PixiSceneView.destroy ← GamePreview.tsx cancelled path) with app#2 live, app#2's next render throws the L-116 error. Sessions where the destroy landed before the survivor's 2nd rebuild did not crash.
  - Deterministic mechanism repro (playwright + swiftshader, two Applications on one page, poke-child per frame to force instruction rebuilds): `a.destroy(true, {children:true})` while b lives → the exact SHELL-1 stack, 100% of runs; `a.destroy({ removeView: true }, {children:true})` → clean.
  - Fix (94033b5, packages/runtime/src/pixi/index.ts): `destroy()` now passes `{ removeView: true }` — canvas detach was the only behavior we wanted from `true`; global pools are page-lifetime by design and must never be released while any other view can be alive. Plus a render-after-destroy guard in `renderOnce()` (`stepFrame` awaits before rendering, so the view can be destroyed mid-await).
  - Stress proof (own vite :5357, ember-horde scratch copy, headless system Chrome + swiftshader, open → 2s → canvas click → 2.6s idle per session, pageerror capture): unfixed HEAD **6/40 sessions (~15%)** threw the exact stack; fixed **0/250 sessions, 0 pageerrors of any kind** (575s run) — >5x the 44 sessions SHELL-1 needed for 5 hits. Runtime suite 462/462, editor suite 1063/1063, runtime+editor typecheck clean, runtime dist + player bundle rebuilt (1.35 MB, under the 1.45 MB budget test).
...
- Disposition: fixed — extended the typing-only guard into `isInteractiveTarget` (keybinds.ts): a superset of `isTypingTarget` that also yields to buttons/links/selects and ARIA role controls (self-or-ancestor via `closest`). `panSpaceKey` now uses it, so a focused button's native Space-activation is never preventDefault'd. Additionally scoped the window Space grab to when the Scene host is visible AND hovered/holding focus (`pannable()` guard + hover ref), so Space is no longer grabbed globally while the user works in another panel. Live-verified: focused toolbar button + Space activates it (opens the New-scene modal); Space still pans when the Scene view is hovered; Space still types in the Code editor. Tests: keybinds.test.ts (isInteractiveTarget), sceneViewHelpers.test.ts (panSpaceKey yields to buttons/links). Files: apps/editor/src/keybinds.ts, apps/editor/src/components/SceneView.tsx.
...
- Observed: Jake's screenshot — the File menu popover renders clipped off the LEFT viewport edge (item labels truncated: "checkpoint", "w changes", "t…", "project" = the RIGHT half of "Save checkpoint / Review changes / Export… / Close project"). Trigger sits near x=0; popover shifted ~one popover-width (200px) LEFT of trigger.
- Expected: popover anchored to its trigger, clamped inside the viewport.
- Disposition: hardened (all hypothesized paths); needs Jake environment confirm — commit <this>.
  Reproduction: NOT reproducible in dev-mode Chrome or headless swiftshader (chrome-for-testing 1223) at widths 420–2560 or devicePixelRatio 1/2 — the in-window MenuBar left-anchors correctly in every case (independently confirmed by the T14 final gate). The exact left-shift Jake saw requires a RIGHT-anchored popover on a near-x=0 trigger, geometry the current tree never produces (MenuBar/SceneMenu both use the default `align:'left'`); it most likely came from a transient Wave-L build state or the packaged macOS app's fallback in-window bar under retina.
  Root cause of the class of bug: MenuButton popovers had NO viewport clamping (unlike ContextMenu). A trigger-anchored `position:absolute` popover with no clamp spills off whichever edge it reaches — CONFIRMED live: SceneMenu's popover clips off the RIGHT edge at width 420 (popRight 579 > vw 420). Same mechanism, either edge.
  Fix (defense in depth, environment-independent): MenuButton now PORTALS its popover to <body> and positions it with measured, viewport-CLAMPED `fixed` coords (new pure helpers `clampAxis` + `menuDropdownPosition` in Menu.tsx — anchor to trigger, clamp both axes to `[margin, viewport-margin]`, flip above when opening below would overflow the bottom). `clampAxis` refuses a negative/too-small start, so the exact Jake left-clip geometry (right-anchor on an x=0 trigger) snaps to the margin instead of off-screen. Portaling also escapes any ancestor overflow clip. Click-outside now treats the portaled popover as "inside". Also hardened `useShowInWindowMenuBar`: a macOS build whose preload lacks `setAppMenu` (stale preload — the likely path to an in-window popover on the packaged mac app) now falls back to the (clamped) in-window bar instead of stranding the user with no menu.
  Verified: live before/after screenshots — SceneMenu clipped-right at W420 → fully on-screen after; File/Edit/View/Help all anchored + on-screen at DPR 1 and 2. Tests: menu.test.tsx `clampAxis` + `menuDropdownPosition` blocks (incl. the near-x=0 right-anchor left-clip case), existing MenuButton real-DOM tests still green through the portal refactor; nativeMenu.test.ts stale-preload fallback case. Files: apps/editor/src/components/ui/Menu.tsx, apps/editor/src/styles/primitives/menu.css, apps/editor/src/menu/nativeMenu.ts.
...
- Disposition: hardened (all hypothesized paths); needs Jake environment confirm — commit <this>.
  Reproduction: NOT reproducible via the File-menu path (or any path) in dev Chrome / headless swiftshader — the ExportDialog is a single always-mounted `<Modal>` whose effect calls `showModal()` on open, and it opens centered + `:modal` + backdrop at widths 420–2560 and DPR 1/2 (independently confirmed by the T14 final gate). There is only ONE open path in the code (both the File menu item and the store share `setExportOpen(true)`), so the "menu path skips showModal" hypothesis does not hold in the current tree.
  Root cause (mechanism, unconfirmed as Jake's trigger): bottom-right is the signature of a `<dialog>` that is `open` but NOT in the top layer — it then lays out in normal flow (`position:absolute`, bottom-right of its DOM parent) instead of UA-centered. That happens if the dialog node is remounted/reparented while open (dialog.open stays true, top-layer status silently lost) or if `showModal()` is skipped. The old CSS leaned ENTIRELY on the UA `:modal` centering with no author fallback, so any drop out of the top layer landed bottom-right.
  Fix (defense in depth): (1) ui.tsx Modal effect now detects open-but-not-modal (`isModalOpen` → `dialog.matches(':modal')`, jsdom-safe fallback to `.open`) and recovers by close()+showModal() to re-enter the top layer. (2) dialogs.css now pins explicit author centering on `dialog.modal` and `dialog.shortcut-sheet` — `position:fixed; inset:0; margin:auto` with `width/height:fit-content` (min/max-width still clamp) — so the box is viewport-centered even in the degraded non-modal frame, no longer dependent on `:modal` matching.
  Verified: live screenshot — Export via File menu opens centered with backdrop at DPR 2; Slice/confirm/New-scene modals and the shortcut sheet unaffected (share the same hardened CSS). Tests: modalShowModal.test.tsx (jsdom) — showModal (not show/attr) called on open, close on close, `isModalOpen` `:modal`/`.open`-fallback, and the open-but-not-modal recovery re-show. Files: apps/editor/src/components/ui.tsx, apps/editor/src/styles/panels/dialogs.css.
